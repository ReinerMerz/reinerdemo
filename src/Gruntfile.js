...
module.exports = function (grunt) {
  ...
  const moduleAliasesToExclude = nucComponent.getAllRequireJsPlugins();
  // RequireJS modules of the components, which we depend on
  const modulesWeDependOn = [
    ...nucComponent.getAllModules(), ...csuiComponent.getAllModules()
  ];

   // RequireJS modules packed into the product bundles
   const requirejsModules = [
    {
      name: 'bundles/ademo-all',
      // Only plugin aliases are needed; modules referenced
      // by component prefixes are excluded by pointing their
      // prefix to "empty:" in the build configuration
  ...
  require('../lib/src/nuc/grunt-tasks/utils/create.build.config')({
    prefix: 'ademo', dependencies: ['csui']
  });
  ...
  grunt.initConfig({
    ...
    replace: {
      ...
      debug: {
        files: [
          {
            src: ['ademo-extensions.json'],
            dest: '../out-debug/ademo-extensions.json'
          }
        ]
      },
      release: {
        files: [
          {
            src: ['ademo-extensions.json'],
            dest: '../out-release/ademo-extensions.json'
          }
        ]
      }
    },
    ...
    copy: {
      module: {
        files: [
          {
            cwd: "../srcmodules/ademo",
            src: "**",
            dest: "../out-module/ademo",
            expand: true
          },
          {
            cwd: "../out-release",
            src: "**",
            dest: "../out-module/ademo/support",
            expand: true
          }
        ]
      }
  ...
  grunt.registerTask('release', [
    ...
    'rtlcss:release', 'jsonlint:release', 'copy'
  ]);
